...
| --- | ------------------------------------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| T1  | User A reads User B's health data                            | Catastrophic               | RLS on every table; `anon` role excluded                                                                                                                                                                                                         | `rls_isolation.sql` — 67 assertions, run in CI from an empty database and against the live project (ADR-0041)                                   |
...
| T9  | Offline log lost                                             | High (trust)               | SQLite-first write + durable outbox + idempotent upsert; a failed pull costs one entity one pass and never the run (ADR-0045)                                                                                                                    | `destructive.test.ts` against a real SQLite engine. **The airplane-mode half is written as a Maestro flow and has never run**                   |
| T10 | Account deleted but data remains                             | High (legal)               | Edge Function deletes the auth user; every user-owned table cascades from it. Client order is server → device → session                                                                                                                          | Verified against the live project (ADR-0042); the device half is unrun                                                                          |
...
| T13 | The delete endpoint is aimed at someone else's account       | Catastrophic               | **There is no user id parameter.** The id comes from `getUser(token)`, validated against the auth server, with the platform's `verify_jwt` in front. An authorisation check can be removed by accident; a capability that was never built cannot | Verified against the live project: a caller naming a second account in the request body deleted only their own (ADR-0042)                       |
| T14 | The service-role key escapes through a log or an error       | Catastrophic               | The key exists only in the Edge Function's environment. Failures log the message and never the user id or the row; the client is told a written sentence rather than the thrown error                                                            | Code review; `secrets.test.ts` scans every tracked file, and the diagnostics panel has its own test that nothing credential-shaped can reach it |
| T15 | A pull cursor skips rows and hides them permanently          | High (trust)               | Keyset on `(updated_at, id)`, strictly greater; the cursor is not advanced past a page that failed to apply                                                                                                                                      | Six regression tests, and measured against the live project — the old cursor pulled 200 of 250 and wedged (ADR-0043)                            |
